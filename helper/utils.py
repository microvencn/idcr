...
from dataset import QuantileTrans


def data_transform(data_name, data, discrete_cols):
...
def create_folder_for_run(runs_folder, experiment_name):
    ...
    # this_run_folder = os.path.join(runs_folder, experiment_name + time.strftime("%Y.%m.%d_%H.%M.%S"))
    this_run_folder = os.path.join(runs_folder, experiment_name)
...
def write_losses(file_name, losses_accu, epoch, duration):
    with open(file_name, 'a', newline='') as csvfile:
        ...
        row_to_write = [epoch] + ['{:.4f}'.format(np.mean(loss_list)) for loss_list in losses_accu.values()] + [
            '{:.0f}'.format(duration)]
        writer.writerow(row_to_write)
...
def get_discrete_cols(data, data_name):
    ...
    if data_name.startswith("law_school"):
        discrete_cols = [
            "race", "sex"
        ]
    elif data_name.startswith("synthetic"):
        discrete_cols = [
            "A"
        ]
    else:
...
def load_causal_graph(data_name, graph_path=None):
    data_folder = path_by_name(data_name)
